...
const FeedbackPage = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto my-10 p-6 bg-white shadow-lg rounded-lg md:p-8 lg:p-10">
      <h2 className="text-2xl md:text-3xl font-bold text-center mb-6 text-gray-800">Frequently Asked Questions</h2>
      <div className="space-y-4">
        {faqs.map((faq, index) => (
          <div key={index} className="border-b border-gray-300">
            <button
              className="w-full text-left py-3 px-4 text-lg md:text-xl font-semibold flex justify-between items-center hover:bg-gray-100 focus:outline-none"
            ...
            {openIndex === index && (
              <div className="px-4 pb-3 text-gray-600 text-sm md:text-base">{faq.answer}</div>
            )}
          </div>
        ))}
      </div>
